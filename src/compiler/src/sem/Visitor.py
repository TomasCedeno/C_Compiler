...
class Visitor(lark.Visitor):
	...
	def declaracao_variaveis(self,tree):
		...
		if _type!="int":
			self.onwarn(
				tree.line,
				f"Tipo de variable {repr(name)} asumido como 'int' porque no puede ser {repr(_type)}."
			)
			_type="int"
		...
		if tree.children[2].type=="END_COMMAND": #regular variable
			...
			if size==0:
				self.onerr(
					tree.line,
					f"El vector {repr(name)} no puede ser definido con tamaño 0."
				)
	...
	def declaracao_retorno(self,tree):
		...
		if parent_function==None: #safe switch
			self.onerr(
				tree.line,
				f"Declaración de retorno en ninguna función."
			)
			return
		...
		if is_return_void:
			if parent_function_var.type!="void":
				self.onerr(
					tree.line,
					f"Retorno void en una función {repr(parent_function_var.type)}"
				)
			else:
		...
		if parent_function_var.type=="void":
			self.onerr(
				tree.line,
				"Retorno 'no void' en una función 'void."
			)
			return
	...
	def declaracao_funcoes(self,tree):
		...
		for param in parametros.find_data("param"):
			...
			if _type!="int":
				self.onwarn(
					tree.line,
					f"Tipo de variable {repr(param_name)} asumido como 'int' porque no puede ser {repr(_type)}."
				)
				_type="int"
	...
	def __var_already_declared(previous):
		...
		return f"{f_or_var} {repr(previous.name)} ya ha sido declarada como {repr(previous.type)} en la línea {previous.line}."
	def variavel(self,tree):
		...
		if not var:
			self.onerr(
				tree.line,
				f"Variable {repr(ID.value)} no declarada."
			)
			return
		if var.is_function():
			self.onerr(
				tree.line,
				f"La función {repr(var.name)} definida en la línea {var.line} está siendo referenciada como una variable."
			)
			return
		var.referenced=True
		tree.entry=var
		if not var.is_vector() and tree.expression.data=="vetor":
			self.onerr(
				tree.line,
				f"{repr(var.name)} fue definida como variable simple en la línea {var.line}, no como vector, imposible indexar."
			)
			return
			...
			if err:
				self.onerr(
					tree.line,
					f"Operación {repr(msg)} no válida para el vector {repr(var.name)} definido en la línea {var.line}."
				)
				return		
		...
		if isinstance(exp,int):
			if exp<0:
				self.onerr(
					tree.line,
					f"Acceso negativo {repr(str(exp))} al vector {repr(ID.value)}."
				)
	def ativacao(self,tree):
		...
		if not var:
			self.onerr(
				tree.line,
				f"La función {repr(ID.value)} no está declarada."
			)
			return
		tree.entry=var
		var.referenced=True
		if var.type=="void":
			...
			if declaracao_expressao.children[0].expression.data!="ativacao":
				self.onerr(
					tree.line,
					f"La función void {repr(var.name)} declarada en la línea {var.line} está siendo llamada dentro de una expresión."
				)
				return
		...
		if not (len(var.args)==len(exp.children) and all(x==y for x,y in zip(are_args_vars,are_exps_vars))):
			...
			self.onerr(
				tree.line,
				f"La función {repr(var.name)} definida en la línea {var.line} fue llamada con variables {called_with if called_with else 'ninguna variable'}, se esperaba {expected if expected else 'ninguna variable'}."
			)
