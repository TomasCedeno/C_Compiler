...
def onerr(code_splitted,line,msg):
	...
	log_err(f"En la línea {repr(line_str)}, número de línea: {line}")
	print(f"\t{msg}")
def onwarn(code_splitted,line,msg):
	line_str=code_splitted[line-1].strip()
	log_war(f"En la línea {repr(line_str)}, número de línea: {line}")
	print(f"\t{msg}")
def sem(code_splitted,fname,tree,complete_tree,no_output,show):
	...
	try:
		...
		log_err("Ninguna función fue declarada, La función void main(void) necesita ser declarada.")
		b=False
	if last_function:
		...
		if (is_main,is_void,does_have_args)!=(True,True,False):
			...
			b=False
			log_err(f"La última función declarada{str1} tiene el nombre main,{str2} es de tipo void, y{str3} parece que tiene argumentos.")
	...
	for entry in symtable.table.values():
		...
		if not entry.referenced:
			onwarn(
				code_splitted,
				entry.line,
				get_msg(entry)+" definida pero nunca utilizada"
			)
		elif entry.is_var() and not entry.initialized:
			onwarn(
				code_splitted,
				entry.line,
				get_msg(entry)+" no fue inicializada"
			)
		if entry.is_function() and entry.type!="void" and not entry.does_return:
			onerr(
				code_splitted,
				entry.line,
				get_msg(entry)+" no retorna valor"
			)
			b=False

	if not no_output:
		for k, v in symtable.table.items():
			print(f"\n{k} -> {v}")
